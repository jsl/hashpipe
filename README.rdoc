...
Finally, when the ActiveRecord model is deleted, the hashed attribute for that model are also deleted automatically.

HashPipe also supports transparent gzip compression/decompression of attribute data and serialization/deserialization 
using Marshal in case you wish to store Ruby objects in the attribute.

HashPipe uses Moneta, which is a universal library for key-value storage systems.  This means that it supports
all of the backend storage types supported by this common library, and that new backends are added regularly.
